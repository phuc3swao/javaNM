 ...
 * @author Admin
 */
@WebServlet(name = "DeleteProduct", urlPatterns = {"/delete_p"})
public class DeleteProduct extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String id_r = request.getParameter("idsp");
        try {
            int id = Integer.parseInt(id_r);
            OrderDAO dao = new OrderDAO();
            Order o = dao.getObyPid(id);
            if(o!=null){
                request.setAttribute("existO", "Sản phẩm còn tồn tại trên đơn hàng, không thể xóa!");
                request.getRequestDispatcher("ThongBao.jsp").forward(request, response);
            }
            ProductDAO pao = new ProductDAO();
            pao.deleteProduct(id);
